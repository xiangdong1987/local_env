...
3. 管理环境内各种配置和日志
## 实现
* 使用docker composer 就行容器编排
* v1版本暂只支持web环境包括如下
    * nginx 
    * fpm
    * redis 
    * mysql
* 此环境只针对windos环境，需要安装[Docker Desktop](https://www.docker.com/products/docker-desktop)    
> 注意docker desktop 和vm虚拟机不兼容使用docker，不能使用虚拟机。docker 应该可以覆盖当前应用场景
* docker-desktop 开启端口监控和文件共享功能
